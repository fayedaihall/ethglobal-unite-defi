...
async function setupEscrowForTesting() {
  ...
  try {
    ...
    try {
      ...
      console.log("✅ Escrow and resolver registered successfully. Tx:", createTx.transaction.hash);
    } catch (error: any) {
      console.log("⚠️ Could not create escrow (contract may not exist), continuing with mock...");
    }
    ...
    console.log("Hash: " + crypto.createHash('sha256').update("mysecret").digest('hex'));
  }
}
